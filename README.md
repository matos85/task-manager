Task Manager API
Task Manager API — это бэкенд-приложение на основе FastAPI для управления задачами с поддержкой CRUD-операций. Каждая задача имеет уникальный идентификатор (UUID), название, описание и статус (создано, в работе, завершено). Проект включает автоматические тесты на pytest, документацию Swagger, поддержку Docker и соответствует стандартам PEP8 и принципам чистого кода.
Особенности

CRUD-операции:
Создание задачи: POST /tasks/
Получение списка задач: GET /tasks/
Получение задачи по UUID: GET /tasks/{task_id}
...
description: строка (1–500 символов)
...
Хранилище: In-memory (для простоты, в продакшене рекомендуется PostgreSQL)


Качество кода: Соответствие PEP8, модульная структура, аннотации типов
...
Клонируйте репозиторий или создайте файлы из предоставленного кода:git clone <repository-url>
cd task-manager
...
Ожидаемый результат:============================= test session starts =============================
collected 12 items
